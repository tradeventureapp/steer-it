# CLAUDE-history.md — Steer It running log (archived)

> This is the full chronological development running log, moved out of `CLAUDE.md`
> (which was ~5,000 lines and mostly this) to keep the main file a usable primer.
> **This is HISTORY.** Much of it narrates code that no longer exists — the pre-rewrite
> `physics.ts` `step()` drift model (p1–p33), the `sim-real` / `sim-real-2` branches,
> and the kinematic `arcadeModel.ts` were all deleted when `physics4.ts` became the sole
> drive model. It is kept for the lessons and the blow-by-blow (physics evolution, the
> circuit/kerb/grid/gravel saga, tuning measurements). For the CURRENT state, read
> `CLAUDE.md`; the distilled lessons live there too. Newest-relevant entries are at the
> bottom (Stee-Rex arcade, Fury 200 EVO).
